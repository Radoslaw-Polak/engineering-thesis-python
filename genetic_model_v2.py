...
def EvolutionaryModel(time_full, voltage_full, time_part, voltage_part, coeffs_ranges, best_fit_coeffs, coeffs_ranges_groups, curve_group,
    ...
    def avgNbestCoeffs(N, aux_ranked_individuals):
        ...
        # best_ranked_individuals = aux_ranked_individuals[-N:]
    ...
    prediction_start = time_part[-1]
    time_part_enlarged = time_part
    voltage_part_enlarged = voltage_part
    model_best_fit = libs.np.poly1d(best_fit_coeffs)
    #mse_limit = calculate_mse(time_full, voltage_full, best_fit_coeffs)
    mse_limit = calculate_mse(time_part_enlarged, voltage_part_enlarged, best_fit_coeffs)
    ...
    for generation in range(n_gens):
        ...
        for cff_individual in coefficient_individuals:
            # obliczanie 
            mse = calculate_mse(time_part_enlarged, model_best_fit(time_part_enlarged), cff_individual)
            rank = 1 / mse

            time_interval = libs.np.arange(prediction_start, time_part_enlarged[-1], 10)
        ...
        ranked_individuals = [ranked_ind for ranked_ind in ranked_individuals if ranked_ind[3]]
        ranked_individuals = sorted(ranked_individuals, key=lambda x: x[0], reverse=True)             
        #print(f"\n=== GEN: {generation} best solution ===")
        print(f'Ranking  najlepszego: {ranked_individuals[0][0]} --- Malejąca: {ranked_individuals[0][3]}')
        ...
        # warunek końca
        temp_model = libs.np.poly1d(ranked_individuals[0][1])
        temp_voltage_part_enlarged = temp_model(time_part_enlarged)
        print(f'Osiągnięto wartość krytyczną: {libs.np.any(temp_voltage_part_enlarged <= 3.2)}')
        # warunkiem koniecznym warunku końca jest osiągnięcie przez aktualny model wielomianu wartości krytycznej napięcia oraz zapewnienie, że ten wielomian będzie funkcją malejącą na poszerzanym przedziale czasu
        
        if libs.np.any(temp_voltage_part_enlarged <= 3.2) and isDecreasingFunction:
            if ranked_individuals[0][0] > rank_limit * limit_rank_threshold and n >= min_gens_to_complete:
        ...
        elif temp_voltage_part_enlarged[-1] > 3.2 and isDecreasingFunction:
            # poszerzamy analizowany zakres czasu do momentu osiągnięcia pewnej maksymalnej wartości skrajnej odpowiadającej w przybliżeniu chwili rozładowania baterii podobnego typu na podobnym etapie jej życia
            temp_list = time_part_enlarged.tolist()
            temp_list = temp_list + [temp_list[-1] + 10 * i for i in range(1, part_time_vector_size + 1)]

            print(f'len time_part: {len(temp_list)}')
            time_part_enlarged = libs.np.array(temp_list) 
        ...
        # Jednak nie była stosowane dokonywanie mutacji wg prawdopodobnieństwa więc zawsze wykonywało się to co jest dla 'else'
        if mutation_with_prob:
            ...
            for _ in range(iter):
                ...
                # time_interval = np.arange(time_part[-1] - 500, time_full[-1], 10) 
                # derivative = np.poly1d(mutated_ind).deriv()
                # isDecreasingFunction = np.all( derivative(time_interval) < 0)
